...
Use global augmentation with `ReactNavigation.RootParamList` derived from `StaticParamList<typeof RootStack>`:
...
5. Prefer plain screen objects by default. Use `createXScreen` helpers such as `createNativeStackScreen` when they make typed screen configs clearer. Verify helper names per navigator.
...
Use plain screen objects by default. Use `createXScreen` helpers only when they make typed screen configs clearer.
...
For navigation access, prefer the default globally typed hooks.
...
Prefer the default globally typed hooks over manual param lists and navigator-specific screen-prop aliases.
...
If you need the ambient root param list elsewhere, use `ReactNavigation.RootParamList` instead of re-deriving it in multiple places.
...
declare global {
  namespace ReactNavigation {
    interface RootParamList extends StaticParamList<typeof ExampleNavigator> {}
  }
}
...
2. Root typing uses `ReactNavigation.RootParamList`, and the augmentation lives next to the root static navigation.
...
7. Prefer plain screen objects by default. Use `createXScreen` helpers when they make typed screen configs clearer.
8. Prefer the globally typed hooks over navigator-specific generics.
